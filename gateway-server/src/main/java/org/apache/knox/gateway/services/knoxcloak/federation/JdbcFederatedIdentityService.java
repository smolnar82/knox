...
public class JdbcFederatedIdentityService implements FederatedIdentityService {
    ...
    private void ensureTablesExist(DatabaseType databaseType) {
        try {
            createTableIfNotExists(FEDERATED_IDENTITY_TABLE_NAME, databaseType.providersTableSql());
            createTableIfNotExists(FEDERATED_IDENTITY_ATTRIBUTES_TABLE_NAME, databaseType.descriptorsTableSql());
        } catch (Exception e) {
