...
public class AuthorizeResource extends PasscodeTokenResourceBase {
    ...
    public Response authCallback() throws Exception {
        ...
        final String federatedIdentityId = persistFederatedIdentity(authorizeRequestMetadata, federatedTokens.getLeft());
        return getAuthCodeFromKnox(authorizeRequestMetadata, Pair.of(federatedIdentityId, federatedTokens.getRight()));
    }
    ...
    private String persistFederatedIdentity(AuthorizeRequestMetadata meta, String federatedIdToken) throws ParseException {
        final JWT jwt = new JWTToken(federatedIdToken);
        final Map<String, String> attributes = new HashMap<>();
        jwt.getJWTClaimsSet().getClaims().forEach((k, v) ->
                attributes.put(k, String.valueOf(v))
        );
        final FederatedIdentity federatedIdentity = new FederatedIdentity(
                meta.getSubject(),                        // internal user id
                "KEYCLOAK",                               // provider
        ...
        return federatedIdentity.getId();
    }
}
