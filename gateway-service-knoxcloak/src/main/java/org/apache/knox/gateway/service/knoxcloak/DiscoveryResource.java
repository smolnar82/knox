...
public class DiscoveryResource {
    ...
    public Response getConfig(@Context UriInfo uriInfo) {
        if (federatedOpConfiguration.isFederatedOpRedirectEnabled()) {
            return redirectToFederatedOp();
        }

        final String baseUrl = uriInfo.getBaseUri().toString();
        final Map<String, Object> config = new HashMap<>();
        config.put("issuer", baseUrl + "knoxcloak");
        config.put("authorization_endpoint", baseUrl .replaceAll("noauth", "sso")+ AuthorizeResource.RESOURCE_PATH);
        config.put("token_endpoint", baseUrl.replaceAll("noauth", "token") + TokenResource.RESOURCE_PATH);
        config.put("userinfo_endpoint", baseUrl.replaceAll("noauth", "token") + UserInfoResource.RESOURCE_PATH);
    ...
    private Response redirectToFederatedOp() {
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            final HttpGet get = new HttpGet(federatedOpConfiguration.getDiscoveryEndpoint());
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity("Failed to fetch OIDC configuration: " + e.getMessage())
                    .build();
        }
    }
}
