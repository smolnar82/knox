...
public class TokenResource extends PasscodeTokenResourceBase {
    ...
    private String generateFederatedIdToken(JWT accessToken, TokenMetadata tokenMetadata) throws TokenServiceException {
        ...
        claims.put("iss", federatedIdentity.getExternalIssuer());
        claims.put("sub", federatedIdentity.getExternalSubject());
    ...
    private JWT parseFederatedIdToken(TokenMetadata tokenMetadata)
            throws TokenServiceException {
        try {
            final String fedIdTokenString = KnoxcloakUtils.joinFederatedToken(tokenMetadata.getMetadataMap(), true);
            return new JWTToken(fedIdTokenString);
        } catch (Exception e) {
            throw new TokenServiceException("Invalid federated ID token", e);
        }
    }

    private String deriveKnoxSubject(JWT fedIdToken) throws TokenServiceException {
        try {
            var claims = fedIdToken.getJWTClaimsSet();
            var issuer = claims.getIssuer();
            var sub = claims.getSubject();

            var md = MessageDigest.getInstance("SHA-256");
            var hash = md.digest((issuer + ":" + sub).getBytes(StandardCharsets.UTF_8));
            return Base64.getUrlEncoder().withoutPadding().encodeToString(hash);

        } catch (Exception e) {
            throw new TokenServiceException("Failed to derive Knox subject", e);
        }
    }

    private Map<String, Object> extractIdentityClaims(JWT fedIdToken) {
        final Map<String, Object> src = fedIdToken.getJWTClaimsSet().getClaims();
        final Map<String, Object> out = new HashMap<>();
        copy(src, out, "preferred_username");
        copy(src, out, "email");
        copy(src, out, "email_verified");
        copy(src, out, "given_name");
        copy(src, out, "family_name");
        copy(src, out, "name");
        copy(src, out, "locale");
        return out;
    }

    private void copy(Map<String, Object> src, Map<String, Object> dst, String key) {
