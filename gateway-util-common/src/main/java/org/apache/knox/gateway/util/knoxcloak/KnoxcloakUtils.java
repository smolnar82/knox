...
public class KnoxcloakUtils {
    ...
    public static Map<String, String> splitFederatedAccessToken(String token) {
        Map<String, String> parts = new LinkedHashMap<>();
        int i = 0, part = 1;
        while (i < token.length()) {
            int end = Math.min(i + CHUNK_SIZE, token.length());
            parts.put("fed_token_" + part++, token.substring(i, end));
            i = end;
        }
        return parts;
    }

    public static String joinFederatedAccessToken(Map<String, String> tokenMetadataMap) {
        return tokenMetadataMap.entrySet().stream()
                .filter(e -> e.getKey().startsWith("fed_token_"))
                .sorted(Map.Entry.comparingByKey(
                        Comparator.comparingInt(k -> Integer.parseInt(k.replace("fed_token_", "")))
                ))
